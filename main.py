...
def main():
    ...
    train_main(agents, 4, [1,2], 4)
    
def train_main(agents, episodes, generations_list, procs=1):
    '''
    ...
    for generation in generations_list:
        ...
        states, actions, statistics = None, [], []
        worlds = []
        pipe_list = []
        ...
        for pipe in pipe_list:
            result = pipe.recv()
            print("recieved results from pipe")
            if states is None:
                states = result[0]
            else:
                states = np.concatenate((states, result[0]))    
            
            actions.extend(result[1])
            statistics.append(result[2])
        ...
        print('States stored')
        pickle.dump(statistics, open('agent_code/my_agent/Training_data/statistics/' + f'{generation:03}' + '_statistics.txt', 'wb'))
...
def train_process(agents, episodes, generation, send_end, proc):
        ...
        print('Attempting send through pipe to main')
        send_end.send([world.states, world.actions, world.statistics])
        world.end()
